...
import RFB, {DataChannel} from '@novnc/novnc/lib/rfb';
import {Base64} from 'js-base64';
import {
  ClientMessage,
  ServerMessage,
  MESSAGE_PASSING_URL,
} from '../../src/webview_shared';

const vscode = acquireVsCodeApi<never>();

// Type-safe wrapper of vscode.postMessage().
function postClientMessage(message: ClientMessage): void {
...
class MessagePassingDataChannel implements DataChannel {
  ...
  readonly protocol = MESSAGE_PASSING_URL;
  ...
  private onMessage(ev: MessageEvent<ServerMessage>): void {
...
function main(): void {
  ...
  window.addEventListener('message', (ev: MessageEvent<ServerMessage>) => {
